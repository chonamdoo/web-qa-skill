...
The user subsequently approved deterministic CLI batch replay after analysis and scenario validation, compact result indexes backed by protected full evidence, and targeted re-exploration on change or ambiguity. This adopts the execution/output separation considered during [OpenCLI research at `8271afc`](https://github.com/jackwener/OpenCLI/tree/8271afc67e8504bda94c147f446ee29775d08274), not its code, dependencies, API shortcuts, click fallbacks, or retry policy. Existing runners remain preferred; this is not a measured token-savings or reliability guarantee.

The user later approved a CLI-first refinement: an optional executable records/evidence gate and selective negative controls for critical E2E tests, while keeping existing runners and one `web-qa` skill. The design followed review of [Reticle at `3a7785d`](https://github.com/reticlehq/reticle/tree/3a7785dda4c56712501da4fbf7626b49368fee8d). No Reticle code, runtime dependency, MCP registration, or engine port is included. The interchange fields, conservative exit policy, and hash checks are this package's implementation choices, not Reticle compatibility promises. Negative-control instructions are a procedure, not evidence that any real product's tests have been validated.
...
| [Playwright CLI](https://playwright.dev/docs/test-cli), [reporters](https://playwright.dev/docs/test-reporters) | Existing-runner batch execution and separate console/file reports. Check installed configuration and supported flags; runner exit and output do not replace this skill's coverage/evidence reconciliation. |
